...
def preprocess_qwen(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, max_len=2048, system_message: str = "You are a helpful assistant.") -> Dict:
    ...
    im_start, im_end, *_ = tokenizer.additional_special_tokens_ids # Qwen3 has more special tokens
...
class LazySupervisedDataset(Dataset):
    ...
    def process_image(self, image_file, overwrite_image_aspect_ratio=None):
        ...
        if image_aspect_ratio == "highres":
            ...
            # error caused by 1x1 image
            if image.size[0] == 1 and image.size[1] == 1:
                print(f"[DEBUG] {image_file} mode={image.mode}, size={image.size}")
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if any(
        ...
        # transformer version issue
        if "qwen" in model_args.model_name_or_path.lower() and not "qwen3" in model_args.model_name_or_path.lower():
...
def train(attn_implementation=None):
    ...
    if not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0:
        if list(pathlib.Path(training_args.output_dir).glob("checkpoint-*")):
            for path in pathlib.Path(training_args.output_dir).glob("checkpoint-*"):
                print('Removing', path)
                if path.is_dir():
                    shutil.rmtree(path)
                else:
