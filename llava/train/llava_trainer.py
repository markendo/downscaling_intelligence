...
class LLaVATrainer(Trainer):
    ...
    # old version of transformers, can use default method
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        if getattr(self.args, "tune_mm_mlp_adapter", False) or (
            ...
            # Saving state for resuming with preemption
            # self._save_optimizer_and_scheduler(output_dir)
            # self._save_scaler(output_dir)
            # self._save_rng_state(output_dir)
            super(LLaVATrainer, self)._save_checkpoint(model, trial) #, metrics) # save whole model for preemption (easiest way)
        else:
